...
> ⚠️ **后台上传上限 15MB**：Keystatic 会把整个文件读进浏览器内存，大文件会导致后台页面崩溃。超过 15MB 的音频（例如常见的高码率 FLAC）请用下面的批量导入命令，后台会给出提示。
...
- **主页内容选择**：站点设置里可以分别挑选「主页展示的文章 / 项目 / 歌曲」；文章与项目会在「灵感与作品」里均匀交错排布（默认 4 篇文章 + 2 个项目），歌曲会显示在首页「音乐」板块（封面、评分、乐评）。
